...
// ============================================================================
// Rhealstone benchmark entry points
// ============================================================================
...
// ============================================================================
// RV64 bare-metal benchmark entry points retargeted for shell execution
// ============================================================================

int dhrystone_run(void);
int coremark_run(void);
void dhry_heap_reset(void);
...
// Forward declarations for local formatting helpers used by shell_put_* hooks.
...
// UART + VRAM 동시 출력.
// Benchmark support files call this symbol so Dhrystone/CoreMark output also
// appears on both UART and HDMI text VRAM.
...
// External print hooks used by Rhealstone benchmark source files.
...
static void cmd_help(int argc, char **argv) {
    ...
    puts_all("  rheal <bench>         - run Rhealstone benchmark\n");
    puts_all("                         switch|preempt|msg|sem|deadlock|intr\n");
    puts_all("  dhry | dhrystone     - run RV64 Dhrystone 2.1\n");
    puts_all("  core | coremark      - run RV64 CoreMark\n");
}
...
static void cmd_rheal(int argc, char **argv) {
    if (argc < 2) {
        puts_all("usage: rheal <switch|preempt|msg|sem|deadlock|intr>\n");
        return;
    }

    if (my_strcmp(argv[1], "switch") == 0) {
        rhealstone_task_switch_start();
    }
    else if (my_strcmp(argv[1], "preempt") == 0) {
        rhealstone_task_preempt_start();
    }
    else if (my_strcmp(argv[1], "msg") == 0 || my_strcmp(argv[1], "message") == 0) {
        rhealstone_message_latency_start();
    }
    else if (my_strcmp(argv[1], "sem") == 0 || my_strcmp(argv[1], "semaphore") == 0) {
        rhealstone_semaphore_shuffle_start();
    }
    else if (my_strcmp(argv[1], "deadlock") == 0) {
        rhealstone_deadlock_break_start();
    }
    else if (my_strcmp(argv[1], "intr") == 0 || my_strcmp(argv[1], "interrupt") == 0) {
        rhealstone_interrupt_latency_start();
    }
    else {
        puts_all("unknown rheal benchmark: ");
        puts_all(argv[1]);
        puts_all("\n");
    }
}

static void cmd_dhrystone(int argc, char **argv) {
    (void)argc;
    (void)argv;

    puts_all("[bench] start RV64 Dhrystone 2.1\n");
    dhry_heap_reset();
    (void)dhrystone_run();
    puts_all("[bench] Dhrystone finished\n");
}

static void cmd_coremark(int argc, char **argv) {
    (void)argc;
    (void)argv;

    puts_all("[bench] start RV64 CoreMark\n");
    (void)coremark_run();
    puts_all("[bench] CoreMark finished\n");
}
...
static const cmd_t cmds[] = {
    ...
    {"rheal",   cmd_rheal},
    {"dhry",    cmd_dhrystone},
    {"dhrystone", cmd_dhrystone},
    {"core",    cmd_coremark},
    {"coremark", cmd_coremark},
    {0, 0}
};
